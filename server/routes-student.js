...
async function runnerPayload(attempt, exam) {
  return {
...
// ---------- available exams ----------
router.get('/exams/available', requireAuth, async (req, res, next) => {
  try {
    const rows = await q.all(`SELECT * FROM exams WHERE published = 1 ORDER BY close_at ASC`);
    const out = [];
    for (const e of rows) {
      const att = await q.get(`SELECT id, status, score FROM attempts WHERE exam_id = ? AND user_id = ?`, e.id, req.user.id);
      const countRow = await q.get(`SELECT COUNT(*) AS n FROM exam_items WHERE exam_id = ?`, e.id);
      out.push({
        ...examMeta(e, req.query.lang),
        state: windowState(e),
        questionCount: Number(countRow.n),
...
router.post('/attempts/start', requireAuth, async (req, res, next) => {
  try {
    ...
    const exam = Number.isInteger(examId) && await q.get(`SELECT * FROM exams WHERE id = ?`, examId);
    ...
    let attempt = await q.get(`SELECT * FROM attempts WHERE exam_id = ? AND user_id = ?`, examId, req.user.id);
    ...
    if (!attempt) {
      ...
      await audit(req.user.id, 'attempt.start', `exam:${examId}`);
      attempt = await q.get(`SELECT * FROM attempts WHERE id = ?`, r.lastInsertRowid);
    } else {
      attempt = await autoSubmitIfDue(attempt);
      if (attempt.status === 'submitted') return bad(res, 'ALREADY_SUBMITTED', 409);
    }

    res.json({ ok: true, ...(await runnerPayload(attempt, exam)) });
  } catch (e) { next(e); }
});

async function ownAttempt(req, res) {
  const id = Number(req.params.id);
  const attempt = Number.isInteger(id) ? await q.get(`SELECT * FROM attempts WHERE id = ?`, id) : null;
...
router.get('/attempts/:id', requireAuth, async (req, res, next) => {
  try {
    const attempt = await ownAttempt(req, res);
    if (!attempt) return;
    const fresh = await autoSubmitIfDue(attempt);
    if (fresh.status === 'submitted') {
      return res.json({ ok: true, submitted: true, attemptId: fresh.id });
    }
    const exam = await q.get(`SELECT * FROM exams WHERE id = ?`, fresh.exam_id);
    res.json({ ok: true, submitted: false, ...(await runnerPayload(fresh, exam)) });
  } catch (e) { next(e); }
});

router.post('/attempts/:id/answer', requireAuth, async (req, res, next) => {
  try {
    const attempt = await ownAttempt(req, res);
    if (!attempt) return;
    if (attempt.status !== 'in_progress') return bad(res, 'ALREADY_SUBMITTED', 409);
    if (Date.now() > Date.parse(attempt.deadline_at)) {
      await autoSubmitIfDue(attempt);
    ...
    await q.run(`UPDATE attempts SET answers = ? WHERE id = ?`, JSON.stringify(answers), attempt.id);
    res.json({ ok: true, savedAt: nowIso() });
  } catch (e) { next(e); }
});

router.post('/attempts/:id/flag', requireAuth, async (req, res, next) => {
  try {
    const attempt = await ownAttempt(req, res);
    if (!attempt) return;
    if (attempt.status !== 'in_progress') return bad(res, 'ALREADY_SUBMITTED', 409);
    ...
    await q.run(`UPDATE attempts SET flagged = ? WHERE id = ?`, JSON.stringify(flagged), attempt.id);
    res.json({ ok: true });
  } catch (e) { next(e); }
});
...
router.post('/attempts/:id/submit', requireAuth, async (req, res, next) => {
  try {
    ...
    const fresh = await submitAttempt(attempt, late ? attempt.deadline_at : null);
    await audit(req.user.id, 'attempt.submit', late ? 'auto' : 'manual');
    res.json({ ok: true, resultId: fresh.id, autoSubmitted: late });
  } catch (e) { next(e); }
});

// ---------- results ----------
router.get('/attempts/:id/result', requireAuth, async (req, res, next) => {
  try {
    const attempt = await ownAttempt(req, res);
    if (!attempt) return;
...
router.get('/my/results', requireAuth, async (req, res, next) => {
  try {
    const rows = await q.all(
      `SELECT a.id AS attempt_id, a.exam_id AS exam_id, a.status, a.started_at, a.submitted_at, a.score,
    ...
    const results = [];
    for (const r of rows) {
      const cnt = await q.get(`SELECT COUNT(*) AS n FROM exam_items WHERE exam_id = ?`, r.exam_id);
      ...
      results.push({
        ...
        score: r.score, total,
        percent: total && r.score != null ? Math.round((r.score / total) * 1000) / 10 : null,
      });
    }
