...
router.get('/stats', requireAdmin, (req, res) => {
  ...
  res.json({
    ok: true,
    usersTotal: one(`SELECT COUNT(*) AS n FROM users`),
    studentsActive: one(`SELECT COUNT(*) AS n FROM users WHERE role='student' AND status='active'`),
    pendingUsers: one(`SELECT COUNT(*) AS n FROM users WHERE status='pending'`),
    itemsActive: one(`SELECT COUNT(*) AS n FROM items WHERE active=1`),
    examsPublished: one(`SELECT COUNT(*) AS n FROM exams WHERE published=1`),
    examsTotal: one(`SELECT COUNT(*) AS n FROM exams`),
    attemptsSubmitted: one(`SELECT COUNT(*) AS n FROM attempts WHERE status='submitted'`),
    attemptsInProgress: one(`SELECT COUNT(*) AS n FROM attempts WHERE status='in_progress'`),
    meanPercent: meanPct,
  });
});
...
router.post('/topics', requireAdmin, (req, res) => {
  const b = req.body || {};
  const th = str(b.nameTh, 80); const en = str(b.nameEn, 80);
  if (!th || !en) return bad(res, 'TOPIC_NAME_REQUIRED');
  const pos = q.get(`SELECT COALESCE(MAX(position),0)+1 AS p FROM topics`).p;
  const r = q.run(`INSERT INTO topics (name_th, name_en, position) VALUES (?, ?, ?)`, th, en, pos);
  audit(req.user.id, 'topic.create', `${th}/${en}`);
...
router.post('/users', requireAdmin, (req, res) => {
  ...
  if (q.get(`SELECT id FROM users WHERE username = ?`, username)) return bad(res, 'USERNAME_TAKEN', 409);
  const salt = makeSalt();
  const r = q.run(
...
router.put('/users/:id', requireAdmin, (req, res) => {
  ...
  const target = Number.isInteger(id) && q.get(`SELECT * FROM users WHERE id = ?`, id);
  ...
  q.run(`UPDATE users SET first_name=?, last_name=?, org=?, email=?, role=?, status=? WHERE id=?`,
    firstName, lastName,
    typeof b.org === 'string' ? b.org.trim().slice(0, 120) : target.org,
    typeof b.email === 'string' ? b.email.trim().slice(0, 120) : target.email,
    role, status, id);
  audit(req.user.id, 'user.update', target.username);
  res.json({ ok: true });
});
...
router.put('/users/:id/password', requireAdmin, (req, res) => {
  ...
  if (password.length < 8) return bad(res, 'PASSWORD_SHORT');
  const salt = makeSalt();
  q.run(`UPDATE users SET pass_hash=?, salt=? WHERE id=?`, hashPassword(password, salt), salt, id);
  audit(req.user.id, 'user.reset_password', target.username);
  res.json({ ok: true });
});
...
router.delete('/users/:id', requireAdmin, (req, res) => {
  ...
  if (target.role === 'admin' && q.get(`SELECT COUNT(*) AS n FROM users WHERE role='admin'`).n <= 1) {
...
// ================= items =================
function validateItemPayload(b) {
  ...
  if (!Number.isInteger(topicId) || !q.get(`SELECT id FROM topics WHERE id=?`, topicId)) return { err: 'TOPIC_INVALID' };
...
router.get('/items', requireAdmin, (req, res) => {
  ...
  const total = q.get(`SELECT COUNT(*) AS n FROM items i WHERE ${whereSql}`, ...params).n;
  const rows = q.all(
...
router.post('/items', requireAdmin, (req, res) => {
  ...
  audit(req.user.id, 'item.create', String(r.lastInsertRowid));
  res.json({ ok: true, id: r.lastInsertRowid });
});

router.put('/items/:id', requireAdmin, (req, res) => {
  const id = Number(req.params.id);
  const item = Number.isInteger(id) && q.get(`SELECT * FROM items WHERE id = ?`, id);
  ...
  audit(req.user.id, 'item.update', String(id));
  res.json({ ok: true });
});
...
router.delete('/items/:id', requireAdmin, (req, res) => {
  ...
  const item = Number.isInteger(id) && q.get(`SELECT * FROM items WHERE id = ?`, id);
  if (!item) return bad(res, 'ITEM_NOT_FOUND', 404);
  const used = q.get(`SELECT COUNT(*) AS n FROM exam_items WHERE item_id = ?`, id).n;
  if (used > 0) {
    q.run(`UPDATE items SET active = 0, updated_at = ? WHERE id = ?`, nowIso(), id);
    audit(req.user.id, 'item.archive', String(id));
    return res.json({ ok: true, archived: true });
  }
  q.run(`DELETE FROM items WHERE id = ?`, id);
  audit(req.user.id, 'item.delete', String(id));
  res.json({ ok: true, archived: false });
});

router.put('/items/:id/restore', requireAdmin, (req, res) => {
  const id = Number(req.params.id);
  q.run(`UPDATE items SET active = 1, updated_at = ? WHERE id = ?`, nowIso(), id);
  audit(req.user.id, 'item.restore', String(id));
  res.json({ ok: true });
});

router.post('/items/import', requireAdmin, (req, res) => {
  ...
  tx(() => {
    list.forEach((raw, i) => {
      try {
        const v = validateItemPayload(raw);
  ...
  audit(req.user.id, 'item.import', `inserted:${inserted}`);
  res.json({ ok: true, inserted, errors });
});

// ================= exams =================
router.get('/exams', requireAdmin, (req, res) => {
  const rows = q.all(`SELECT * FROM exams ORDER BY created_at DESC`);
  res.json({
    ok: true,
    exams: rows.map((e) => ({
      ...e, published: !!e.published, shuffle: !!e.shuffle,
      itemCount: q.get(`SELECT COUNT(*) AS n FROM exam_items WHERE exam_id=?`, e.id).n,
      attemptCount: q.get(`SELECT COUNT(*) AS n FROM attempts WHERE exam_id=? AND status='submitted'`, e.id).n,
    })),
  });
});
...
router.post('/exams/create', requireAdmin, (req, res) => {
  const b = req.body || {};
  ...
  if (b.mode === 'manual') {
    ...
    const found = picked.filter((id) => q.get(`SELECT id FROM items WHERE id=? AND active=1`, id));
  ...
  tx(() => buildExamItems(r.lastInsertRowid, picked, false));
  audit(req.user.id, 'exam.create', `${r.lastInsertRowid} items:${picked.length}`);
  res.json({ ok: true, id: r.lastInsertRowid, itemCount: picked.length, shortfall });
});

router.put('/exams/:id/publish', requireAdmin, (req, res) => {
  const id = Number(req.params.id);
  const exam = Number.isInteger(id) && q.get(`SELECT * FROM exams WHERE id = ?`, id);
  if (!exam) return bad(res, 'EXAM_NOT_FOUND', 404);
  const publish = (req.body || {}).published ? 1 : 0;
  if (publish) {
    const count = q.get(`SELECT COUNT(*) AS n FROM exam_items WHERE exam_id = ?`, id).n;
    if (!count) return bad(res, 'EXAM_HAS_NO_ITEMS', 409);
  }
  q.run(`UPDATE exams SET published = ? WHERE id = ?`, publish, id);
  audit(req.user.id, publish ? 'exam.publish' : 'exam.unpublish', String(id));
  res.json({ ok: true });
});
...
router.delete('/exams/:id', requireAdmin, (req, res) => {
  ...
  const exam = Number.isInteger(id) && q.get(`SELECT * FROM exams WHERE id = ?`, id);
  if (!exam) return bad(res, 'EXAM_NOT_FOUND', 404);
  const attempts = q.get(`SELECT COUNT(*) AS n FROM attempts WHERE exam_id = ?`, id).n;
  if (attempts > 0) return bad(res, 'EXAM_HAS_ATTEMPTS', 409);
  q.run(`DELETE FROM exams WHERE id = ?`, id);
  audit(req.user.id, 'exam.delete', String(id));
  res.json({ ok: true });
});

router.get('/exams/:id/roster', requireAdmin, (req, res) => {
  const id = Number(req.params.id);
  const exam = Number.isInteger(id) && q.get(`SELECT * FROM exams WHERE id = ?`, id);
  if (!exam) return bad(res, 'EXAM_NOT_FOUND', 404);
  const totalQ = q.get(`SELECT COUNT(*) AS n FROM exam_items WHERE exam_id = ?`, id).n;
  const rows = q.all(
  ...
  res.json({ ok: true, exam: { ...examMetaOut(exam), totalQuestions: totalQ }, roster, serverNow: nowIso() });
});
...
router.get('/exams/:id/item-analysis', requireAdmin, (req, res) => {
  const id = Number(req.params.id);
  const exam = Number.isInteger(id) && q.get(`SELECT * FROM exams WHERE id = ?`, id);
  if (!exam) return bad(res, 'EXAM_NOT_FOUND', 404);
  const submitted = q.all(`SELECT answers FROM attempts WHERE exam_id = ? AND status = 'submitted'`, id);
  const withCorrect = q.all(
...
router.get('/scores', requireAdmin, (req, res) => {
  ...
  const out = rows.map((r) => {
    const total = q.get(`SELECT COUNT(*) AS n FROM exam_items WHERE exam_id = ?`, r.exam_id).n;
...
router.get('/reports/trend', requireAdmin, (req, res) => {
  const rows = q.all(
